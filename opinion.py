...
def format_opinion(op: dict | None) -> str:
    ...
    L.append("  Детерминированная свёртка перечисленных выше фактов по заранее")
    L.append("  заданным весам — не бэктестированный прогноз и не лицензированная")
    L.append("  консультация; веса выбраны рассуждением, как и в cluster.score_signal.")
    L.append("  Новости здесь не оцениваются баллом (см. модуль) — для разбора,")
    L.append("  включающего чтение новостей и качественную оценку движения, нужно")
    L.append("  спросить Claude напрямую: это не автоматизировано (заняло бы вызов")
    L.append("  API, которого сознательно нет в этом пайплайне).")
    return "\n".join(L)
